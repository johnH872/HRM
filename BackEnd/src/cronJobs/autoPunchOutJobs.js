...
async function handleAutoPunchOutJob() {
    try {
        ...
        UPDATE attendances 
        SET punchoutDate = STR_TO_DATE(CONCAT(DATE(punchinDate), '13:00:00'), '%Y-%m-%d %T'),
            punchoutTime = UNIX_TIMESTAMP(ADDTIME(STR_TO_DATE(CONCAT(DATE(punchinDate), ' 13:00:00'), '%Y-%m-%d %T'), '07:00:00.000000'))*1000,
            punchoutOffset = -420,
